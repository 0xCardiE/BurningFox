# Security comparison: BurnBox wallet types vs MetaMask

Side-by-side view of **every BurnBox account type** against the closest **MetaMask** equivalent. BurnBox is a Chrome MV3 developer / burner wallet; MetaMask is a general-purpose browser hot wallet. Hardware rows describe keys that never leave the device.

Sources for BurnBox: `src/lib/vault.ts`, `src/lib/walletCore.ts`, `src/lib/walletManager.ts`, `src/lib/ledger.ts`, `src/lib/trezor.ts`, `src/background.ts`, `src/lib/providerRpc.ts`, `public/manifest.json`.

---

## Wallet types covered

| BurnBox type | How you get it | Closest MetaMask equivalent |
|--------------|----------------|-----------------------------|
...
| Topic | MetaMask (software / SRP) | BurnBox local seed | BurnBox local private key | MetaMask + Ledger/Trezor | BurnBox Ledger / Trezor |
|-------|---------------------------|--------------------|---------------------------|--------------------------|-------------------------|
| **Custody class** | Hot software | Hot software | Hot software | Cold keys, hot UI | Cold keys, hot UI |
| **Secret at rest** | Encrypted vault (SRP + derived keys) | Encrypted vault (`keys` + optional `mnemonic`) in `chrome.storage.local` | Encrypted vault (`keys` only) | Device holds seed; MM stores pubkey metadata | Device holds seed; BurnBox stores address / path / label only |
...
| **Auto-lock default** | User setting | **Off** by default | Same | Same as MM software session | Same BurnBox auto-lock (session metadata) |
| **Close UI = locked?** | No | No | No | No | No |
| **Browser restart** | Relock; vault remains | Relock; vault remains | Same | Relock MM; device unchanged | Relock BurnBox; device unchanged |
...
| **Intended use** | General self-custody | Dev / burner HD wallet | Dev / single-key burner | Higher-security self-custody | Same class as MM+HW inside BurnBox UX |
...
**BurnBox local seed** matches this *model* (create/import phrase, derive `m/44'/60'/0'/0/n`) but keeps BurnBox’s **speed defaults**: Turbo auto-sign, optional auto-lock off, and plaintext **active** private key in `chrome.storage.session` while unlocked.

### 2. BurnBox local seed (HD)
...
### 3. BurnBox local private key (and MetaMask “Import private key”)
...
- Same BurnBox unlocked-session caveats as seed accounts for that active key.
...
| Threat | Local seed / PK (BurnBox) | Ledger / Trezor (BurnBox or MM) |
|--------|---------------------------|----------------------------------|
| Stolen locked browser profile | Password-guess vault | Address metadata only; funds need device |
| Stolen unlocked browser profile | **High** — session PK (+ seed in BurnBox UI memory if present) | Attacker can *prompt* signs; user must approve on device |
| Malicious dApp | Turbo may auto-sign (BurnBox); enable Normal | Device shows tx; user is last line of defense |
...
1. **Burner / test funds** — BurnBox local seed or private key is fine; keep Turbo if you want speed; enable auto-lock if the machine is shared.
2. **Real funds in browser** — Prefer **Ledger or Trezor** (BurnBox or MetaMask), Normal confirmations, verify every device prompt.
3. **MetaMask vs BurnBox software** — Same custody *class*; MetaMask wins on conservative defaults (confirm-every-tx, mature SRP UX). BurnBox wins on multi-RPC / LiFi / multi-send / burner workflow — accept hotter unlocked-state tradeoffs.
4. **Seed vs private key in BurnBox** — Prefer **seed** if you want MetaMask-like multiple derived accounts from one backup. Use **private key** for disposable single-account burners.
...
|-----------|----------------------|--------|
| Key never in browser | Ledger / Trezor (BurnBox or MM) | Best for value |
| Familiar HD recovery | MetaMask SRP ≈ BurnBox local seed | BurnBox seed is newer; verify backups |
| Disposable account | BurnBox private key | No phrase to leak across accounts |
| Default signing safety | MetaMask | BurnBox Turbo is convenience-first |
| Unlocked session on disk | MetaMask (typically memory) | BurnBox stores active PK in session storage |

For material holdings: **hardware wallet + confirm-on-device**, whether through MetaMask or BurnBox. Treat BurnBox software accounts as **hot burners** unless you consciously harden (Normal mode, auto-lock, strong password, separate browser profile).
